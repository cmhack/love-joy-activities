	...
	if(getURLParameter('cardID') != null){
		...
		if($scope.cardID == 1){
			...
			$.ajax({
				...
				success: function(data){
					...
					$scope.firstIndex = $scope.list1[0]._Index;
					$scope.lastIndex = $scope.list2[$scope.list2.length-1]._Index;
		...
		if($scope.cardID == 6){
			...
			$.ajax({
				...
				/*	var num = ((data.root.Topic.length - 39) / 2) + 39;
					...
					//-- store 10 items into list 1
					for(var i=num; i<data.root.Topic.length; i++){
						$scope.list2.push(data.root.Topic[i]);
					}
					*/
					$scope.firstIndex = $scope.list1[0]._Index;
